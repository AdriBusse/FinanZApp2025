...
export default function SavingsList() {
  ...
  const { isSpeedDialOpen, toggleSpeedDial, openCreateDepotModal, isCreateDepotModalOpen, closeCreateDepotModal } = useSavingsUIStore();
  ...
  return (
    <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('SavingTransactions', { depotId: item.id })}
            >
              <View>
                <Text style={styles.depotName}>{item.name}</Text>
                <Text style={[styles.depotSub, { color: (item.sum ?? 0) >= 0 ? '#16a34a' : '#ef4444' }]}>Total: {(item.sum ?? 0).toLocaleString()}</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Text style={styles.depotBalance}>{(item.balance ?? 0).toLocaleString()}</Text>
                <TouchableOpacity
                  accessibilityLabel="Delete depot"
                  onPress={() => {
                    Alert.alert('Delete Depot', `Are you sure you want to delete "${item.name}"?`, [
                      { text: 'Cancel', style: 'cancel' },
                      { text: 'Delete', style: 'destructive', onPress: async () => { try { await deleteSavingDepot(item.id); } catch {/* no-op */} } },
                    ]);
                  }}
      ...
      <FABSpeedDial
        ...
        position="left"
        actions={[{ label: 'New Depot', onPress: openCreateDepotModal, color: '#2563eb' }]}
      />

      {/* Create Depot Modal */}
      <CreateDepotModal visible={isCreateDepotModalOpen} onClose={closeCreateDepotModal} />
    </View>
  );
}

function CreateDepotModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
...
const styles = StyleSheet.create({
  ...
  depotItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 14, backgroundColor: '#1e212b', borderRadius: 12 },
  sep: { height: 12 },
  ...
  fabContainer: { position: 'absolute', left: 16, bottom: 24, alignItems: 'flex-start' },
  fab: { width: 56, height: 56, borderRadius: 28, backgroundColor: '#22c55e', alignItems: 'center', justifyContent: 'center', elevation: 6 },
  fabText: { color: '#fff', fontSize: 28, fontWeight: '700', marginTop: -2 },
  speedDialMenu: { marginBottom: 8 },
  smallButton: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 16, marginBottom: 8 },
  smallButtonText: { color: '#fff', fontWeight: '700' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
  modalCard: { backgroundColor: '#111827', padding: 16, borderRadius: 12, width: '86%' },
